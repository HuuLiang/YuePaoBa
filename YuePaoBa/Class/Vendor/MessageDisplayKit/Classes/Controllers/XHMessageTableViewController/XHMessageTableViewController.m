...
static NSString *robotPushIdentifier = @"RobotPushInfoIdentifier";
...
- (void)viewDidLoad {
    ...
    // 初始化消息页面布局
    [self.messageTableView registerClass:[YPBRobotTableViewCell class] forCellReuseIdentifier:robotPushIdentifier];
    [self initilzer];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XHMessage *chatMessage = self.messages[indexPath.row];
    if ([chatMessage.type isEqual:@(YPBChatMessageTypeRobotPush)]) {
        //显示红娘信息
        YPBRobotTableViewCell *robotCell = [tableView dequeueReusableCellWithIdentifier:robotPushIdentifier];
        if (!robotCell) {
            robotCell = [[YPBRobotTableViewCell alloc] initWithReuseIdentifier:robotPushIdentifier];
        }
        
        if ([self.delegate respondsToSelector:@selector(configRobotCell:atIndexPath:)]) {
            [self.delegate configRobotCell:robotCell atIndexPath:indexPath];
        }
        
        [robotCell setBackgroundColor:tableView.backgroundColor];
        return robotCell;
    } else {
        //显示普通的聊天信息
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([chatMessage.type isEqual:@(YPBChatMessageTypeRobotPush)]) {
        if ([chatMessage.text isEqualToString:kRobotPushWelcomeIdentifier]) {
            return 270;
        } else if ([[chatMessage.text componentsSeparatedByString:YPBROBOTID][0] isEqualToString:kRobotPushGreetIdentifier]) {
            return 120;
        } else {
            return 0;;
        }
