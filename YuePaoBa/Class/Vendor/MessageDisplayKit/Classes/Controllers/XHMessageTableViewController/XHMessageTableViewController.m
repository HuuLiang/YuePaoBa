...
- (void)viewDidLoad {
    ...
    // 初始化消息页面布局
    [self.messageTableView registerClass:[YPBBlacklistSystemInfoCell class] forCellReuseIdentifier:@"BlacklistSystemInfoCell"];
    [self initilzer];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XHMessage *aChatMessage = self.messages[indexPath.row];
    
    if ([aChatMessage.type isEqual:@(YPBChatMessageTypeSystemInfo)]) {
        
        static NSString *BlacklistIdentifier = @"BlacklistSystemInfoCell";
        
        YPBBlacklistSystemInfoCell * cell = [tableView dequeueReusableCellWithIdentifier:BlacklistIdentifier];
        if (!cell) {
            cell = [[YPBBlacklistSystemInfoCell alloc] initWithReuseIdentifier:BlacklistIdentifier];
        }
        if ([self.delegate respondsToSelector:@selector(configBlacklistCell:atIndexPath:)]) {
            [self.delegate configBlacklistCell:cell atIndexPath:indexPath];
        }
