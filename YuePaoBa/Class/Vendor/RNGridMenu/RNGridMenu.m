...
- (void)layoutSubviews {
    ...
    BOOL hasImage = self.imageView.image != nil || self.imageView.sd_imageURL != nil;
...
- (instancetype)initWithImageUrl:(NSURL *)imageUrl title:(NSString *)title action:(dispatch_block_t)action {
    if (self = [super init]) {
...
- (instancetype)initWithImageUrl:(NSURL *)imageUrl title:(NSString *)title {
    return [self initWithImageUrl:imageUrl title:title action:nil];
}

- (instancetype)initWithImageUrl:(NSURL *)imageUrl {
    return [self initWithImageUrl:imageUrl title:nil action:nil];
}
...
- (BOOL)isEqual:(id)object {
    ...
    return ((self.title == [object title] || [self.title isEqualToString:[object title]]) &&
            (self.image == [object image]) && (self.imageUrl == [object imageUrl] || [self.imageUrl isEqual:[object imageUrl]]));
}
...
- (instancetype)initWithItems:(NSArray *)items {
    if ((self = [super init])) {
        ...
        BOOL hasImages = [items filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(RNGridMenuItem *item, NSDictionary *bindings) {
            return item.image != nil || item.imageUrl != nil;
        }]].count > 0;
...
- (void)viewWillLayoutSubviews {
    ...
    footerFrame.size.width = self.menuView.bounds.size.width;
    footerFrame.origin = CGPointMake(0, CGRectGetMaxY(self.menuView.bounds)-footerFrame.size.height);
...
- (void)layoutAsList {
    ...
    CGFloat footerOffset = CGRectGetHeight(self.footerView.bounds);
    self.menuView.transform = CGAffineTransformIdentity;
    self.menuView.frame = [self menuFrameWithWidth:width height:height center:self.menuCenter headerOffset:headerOffset footerOffset:footerOffset];
...
- (void)layoutAsGrid {
    ...
    CGFloat footerOffset = self.footerView.bounds.size.height;
    self.menuView.transform = CGAffineTransformIdentity;
    self.menuView.frame = [self menuFrameWithWidth:width height:height center:self.menuCenter headerOffset:headerOffset footerOffset:footerOffset];
    
    for (NSInteger i = 0; i < rowCount; i++) {
        NSInteger normalRowLength = ceilf(itemCount / (CGFloat)rowCount);
        NSInteger rowStartIndex = i * normalRowLength;

        NSInteger currentRowLength = normalRowLength;
        if ((i + 1) * normalRowLength > itemCount) {
            currentRowLength = itemCount - i * normalRowLength;
        }
        NSArray *subItems = [self.itemViews subarrayWithRange:NSMakeRange(rowStartIndex, currentRowLength)];
        CGFloat itemWidth = floorf(width / (CGFloat)normalRowLength);
...
- (void)showInViewController:(UIViewController *)parentViewController center:(CGPoint)center {
    [self showInViewController:parentViewController center:center withCompletionHandler:nil];
}

- (void)showInViewController:(UIViewController *)parentViewController center:(CGPoint)center withCompletionHandler:(dispatch_block_t)handler {
    ...
    [self showAnimated:YES withComletionHandler:handler];
}

- (void)showAnimated:(BOOL)animated withComletionHandler:(dispatch_block_t)handler {
    ...
    if (self.footerView) {
        [self.menuView addSubview:self.footerView];
    }
...
- (CGRect)menuFrameWithWidth:(CGFloat)width height:(CGFloat)height center:(CGPoint)center headerOffset:(CGFloat)headerOffset footerOffset:(CGFloat)footerOffset {
    height += headerOffset;
    height += footerOffset;
