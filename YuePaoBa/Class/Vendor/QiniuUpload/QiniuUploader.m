...
- (AFHTTPRequestOperation*)QiniuOperation:(NSInteger)index file:(QiniuFile *)file
{
    ...
    parameters[@"token"] = accessToken?:[[QiniuToken sharedQiniuToken] uploadTokenWithKey:file.key];
    NSMutableURLRequest *request = [operationManager.requestSerializer
                                     multipartFormRequestWithMethod:@"POST"
                                                          URLString:kQiniuUpUploadUrl
                                                         parameters:parameters
                                          constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                                         [formData appendPartWithFileData:file.rawData name:@"file" fileName:@"file" mimeType:theFile.mimeType];
                                     } error:nil];
...
- (void)getQiniuFileSourceDataAndCreateOperation:(NSInteger)index
{
    ...
    if (qiniuFile.rawData) {
        AFHTTPRequestOperation *theOperation = [self QiniuOperation:index file:qiniuFile];
        ...
        [library assetForURL:qiniuFile.assetURL resultBlock:^(ALAsset *asset) {
            ...
            if (self.processAsset) {
                qiniuFile.rawData = self.processAsset(asset);
            }else{
